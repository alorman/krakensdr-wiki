...
To proceed with this; you need to install [RTL-SDR](https://github.com/osmocom/rtl-sdr/tree/master).  
Test with `rtl_test`, which should display the device index and serial number of each of the 5x RTL-SDRs on the system.  

First ensure that no service is claiming the RTL-SDR devices. Then invoke `rtl_biast` as below:  

To turn off the noise source:  
`sudo rtl_biast -d 1 -g 0 -b 0`  

Where:  
`-d 1` is device 1. Do not change this.  
`-g 0` is GPIO 0, for the noise source.
`-b 0` is off. `-b 1` would be on.

To turn on and off the individual bias-T supplies:  
`sudo rtl_biast -d 1 -g 1 -b 1`  

Where:  
`-d 1` is device 1. Again, do not change this.
`-g 1` is GPIO 1, which is the bias-T for channel 0. This increments to `-g 5`, which is channel 4.  
`-b 1` is to turn on. `-b 0` would be off.

*Note: these settings do not persist on reboot and cannot be made to do so. Use a script or service instead if this feature is desired.

# Serial Numbers of Receivers
...
You need to have the [RTL-SDR](https://github.com/osmocom/rtl-sdr/tree/master) tools installed, above.  
You can invoke `rtl_eeprom` to reprogram the serial numbers of the receivers.  

You can change the serial numbers to either a 4 or 8 digit number, either one seems to work. Change with:  
`rtl_eeprom -d 0 -s 00001090`

Where:
`-d 0` is device index 0. Increment for each desired change.  
`-s XXXX` is the desired serial number, where XXXX is the new desired serial.  

*Do not change the bias-T settings in with the rtl_eeprom tool. It is not supported. 
