...
* Odroid N2/+ - Note: You must use the **BOTTOM USB port that is closest to the DC power barrel jack**. All other USB ports do not work for high data rate devices on this machine. Also requires a high quality USB cable. Also as this device does not have built in WiFi it relies on Ethernet or a USB WiFi stick. When using a USB WiFi stick there will be sample alignment glitches every now and then (we count about one USB glitch every 12-24 hours). This results in the DOA/PR results going bad. If you have periodic recalibration turned ON (the default), it's not a huge deal, but you could be about 5 minutes with bad results.
